...
PLAN = [
    ...
    ("2027-02", 22000, 18000, 4000, 2.8, "누적 흑자 전환 확인"),
...
def build_summary() -> str:
    rows = [
        ...
        row_xml(3, ["기준일", "2026-08-18", "현재 Total Revenue", 25439.99, "현재 광고비", 35235.56], [3, 7, 3, 4, 3, 4], height=24),
        row_xml(4, ["현재 관리 ROI", 0.72, "마감 누적 적자(7월)", -66111.69, "8월 포함 보수적 누적", STARTING_CUMULATIVE], [3, 5, 3, 4, 3, 4], height=24),
        row_xml(6, ["경영 결론", "", "", "", "", ""], [2, 2, 2, 2, 2, 2], height=24),
        row_xml(7, ["월 흑자 목표", "2026년 10월", "누적 흑자 목표", "2027년 2월", "ROI 3.0 목표", "2027년 3월"], [3, 10, 3, 10, 3, 10], height=28),
...
def build_monthly_plan() -> str:
    headers = ["월", "최대 승인액", "우선 집행액", "조건부 예비액", "목표 ROI", "매출 목표", "비광고비 추정", "예상 월손익", "예상 누적손익", "상태", "집행 조건"]
    rows = [
        row_xml(1, ["2026.09~2027.03 월별 실행 계획", "", "", "", "", "", "", "", "", "", ""], [1] * 11, height=30),
        row_xml(2, ["시작 누적손익", STARTING_CUMULATIVE, "비광고비율", NON_AD_RATIO, "ROI 정의", "Total Revenue ÷ 광고비", "", "", "", "", ""], [3, 11, 3, 6, 3, 7, 0, 0, 0, 0, 0], height=24),
    ...
    for offset, (month, max_budget, initial, reserve, roi, gate) in enumerate(PLAN, start=5):
        ...
        prev_cum = "$B$2" if offset == 5 else f"I{offset - 1}"
        formulas = {
            ...
            9: (f"{prev_cum}+H{offset}", cumulative),
        }
        status = "누적 흑자" if cumulative >= 0 else ("월 흑자" if monthly_profit >= 0 else "전환 단계")
        values = [month, max_budget, initial, reserve, roi, revenue, non_ad, monthly_profit, cumulative, status, gate]
        styles = [7, 4, 4, 4, 5, 4, 4, 4, 8 if cumulative >= 0 else 4, 10 if cumulative >= 0 else 7, 7]
    ...
    return worksheet(rows, [13, 16, 16, 16, 13, 16, 17, 17, 17, 15, 34], ["A1:K1"], "A5", "A4:K11", conditional)
...
def build_gates() -> str:
    ...
    rows.extend([
        ...
        row_xml(12, ["", "9월 광고비는 최대 $32,000으로 승인받되 $24,000만 우선 집행합니다. 목표 ROI와 상품별 기여이익을 충족할 때만 예비액 $8,000을 단계적으로 사용합니다. 2027년 2월 누적 흑자, 3월 ROI 3.0을 목표로 관리합니다.", "", ""], [0, 9, 9, 9], height=58),
    ])
...
def build_sources() -> str:
    data = [
        ...
        ("7월까지 누적", "assets/monthly-data.js 및 월별 Finance 원본", "2025-11~2026-07", "추정 누적손익 -$66,111.69"),
        ("비광고비율", "Admin 비용 스냅샷", "현재 계획 가정", "$13,559 ÷ $72,904.18 = 18.598%"),
        ("8월 포함 누적", "정산액과 8/17까지 광고비 결합", "컷오프 정렬", "보수적 누적 -$89,794.06"),
    ]
    rows = [
    ...
    rows.extend([
        row_xml(10, ["검증 제한", "", "", ""], [2, 2, 2, 2], height=24),
        row_xml(11, ["", "8월 매출과 광고비의 종료일이 하루 다릅니다. 누적 손익은 의사결정용 보수적 추정이며 월 마감 확정치가 아닙니다.", "", ""], [0, 9, 9, 9], height=42),
        row_xml(12, ["", "제품 원가·시딩·수수료·물류비의 실제 월 마감 원장이 연결되면 비광고비 추정치를 실제값으로 교체해야 합니다.", "", ""], [0, 9, 9, 9], height=42),
    ])
    return worksheet(rows, [18, 45, 23, 41], ["A1:D1", "A10:D10", "B11:D11", "B12:D12"])
